...
const Certificates = ({ title, data }: CertificatesProps) => {
    return (
        <section
            id="certificates"
            className="py-12 md:py-16 bg-gradient-to-b from-primary/5 to-background"
        >
            <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
                ...
                <div
                    className={`grid gap-8 ${data.length === 1 ? "grid-cols-1 max-w-md mx-auto" : "grid-cols-1 md:grid-cols-2"}`}
                >
                    {data.map((cert, index) => (
                        <a
                            key={index}
